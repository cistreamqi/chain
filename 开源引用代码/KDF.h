#pragma once
#define SM2_WORDSIZE1 8
#define SM2_NUMBITS1 256
#define SM2_NUMWORD1 (SM2_NUMBITS1/SM2_WORDSIZE1) //32
/************************************************************************
FileName:
KDF.h
Version:
...
void SM3_done(SM3_STATE *md, unsigned char hash[])
{
	...
	*/
	...
	BigEndian(hash, SM3_len / 8, hash);//if CPU uses little-endian, BigEndian function is anecessary call
}
/******************************************************************************
Function: SM3_256
...
void SM3_KDF(unsigned char Z[], unsigned short zlen, unsigned short klen, unsigned char K[])
{
	...
	SM3_STATE md;
	unsigned char Ha[SM2_NUMWORD1];
	unsigned char ct[4] = { 0,0,0,1 };
	bitklen = klen * 8;
	if (bitklen%SM2_NUMBITS1)
		t = bitklen / SM2_NUMBITS1 + 1;
	else
		t = bitklen / SM2_NUMBITS1;
	//s4: K=Ha1||Ha2||...
	for (i = 1; i<t; i++)
	{
		//s2: Hai=Hv(Z||ct)
		...
		SM3_process(&md, ct, 4);
		SM3_done(&md, Ha);
		memcpy((K + SM2_NUMWORD1*(i - 1)), Ha, SM2_NUMWORD1);
		if (ct[3] == 0xff)
		{
			ct[3] = 0;
			if (ct[2] == 0xff)
	...
	if (bitklen%SM2_NUMBITS1)
	{
		i = (SM2_NUMBITS1 - bitklen + SM2_NUMBITS1*(bitklen / SM2_NUMBITS1)) / 8;
		j = (bitklen - SM2_NUMBITS1*(bitklen / SM2_NUMBITS1)) / 8;
		memcpy((K + SM2_NUMWORD1*(t - 1)), Ha, j);
	}
	else
	{
		memcpy((K + SM2_NUMWORD1*(t - 1)), Ha, SM2_NUMWORD1);
	}
}