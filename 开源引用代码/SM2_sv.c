...
unsigned char SM2_p1[32] ={ 0xff,0xff,0xff,0xfe,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00, 0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xff };
unsigned char SM2_a1[32] ={ 0xff,0xff,0xff,0xfe,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00, 0xff,0xff,0xff,0xff, 0xff,0xff,0xff,0xfc };
unsigned char SM2_b1[32] = { 0x28,0xe9,0xfa,0x9e, 0x9d,0x9f,0x5e,0x34,
0x4d,0x5a,0x9e,0x4b,0xcf,0x65,0x09,0xa7,
0xf3,0x97,0x89,0xf5, 0x15,0xab,0x8f,0x92, 0xdd,0xbc,0xbd,0x41,0x4d,0x94,0x0e,0x93 };
unsigned char SM2_Gx1[32] = { 0x32,0xc4,0xae,0x2c,
0x1f,0x19,0x81,0x19,0x5f,0x99,0x04,0x46,0x6a,0x39,0xc9,0x94,
0x8f,0xe3,0x0b,0xbf,0xf2,0x66,0x0b,0xe1,0x71,0x5a,0x45,0x89,0x33,0x4c,0x74,0xc7 };
unsigned char SM2_Gy1[32] = { 0xbc,0x37,0x36,0xa2,0xf4,0xf6,0x77,0x9c,0x59,0xbd,0xce,0xe3,0x6b,0x69,0x21,0x53,
0xd0,
0xa9,0x87,0x7c,0xc6,0x2a,0x47,0x40,0x02,0xdf,0x32,0xe5,0x21,0x39,0xf0,0xa0 };
unsigned char SM2_n1[32] ={ 0xff,0xff,0xff,0xfe,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
0x72,0x03,0xdf,0x6b,0x21,0xc6,0x05,0x2b,0x53,0xbb,0xf4,0x09,0x39,0xd5,0x41,0x23 };

/****************************************************************
Function: SM2_Init
Description: Initiate SM2 curve///初始化SM2椭圆曲线
...
int SM2_Init()
{
	...
	b = mirvar(0);
	n = mirvar(0);
	bytes_to_big(SM2_n1UMWORD, SM2_Gx1, Gx);
	bytes_to_big(SM2_n1UMWORD, SM2_Gy1, Gy);
	bytes_to_big(SM2_n1UMWORD, SM2_p1, p);
	bytes_to_big(SM2_n1UMWORD, SM2_a1, a);
	bytes_to_big(SM2_n1UMWORD, SM2_b1, b);
	bytes_to_big(SM2_n1UMWORD, SM2_n1, n);
...
/****************************************************************
Function: Test_Point///测试基点是否在曲线上
...
/****************************************************************
Function: SM2_KeyGeneration///利用私钥和基点生成公钥
...
int SM2_KeyGeneration(unsigned char PriKey[], unsigned char Px[], unsigned char Py[])
{
	...
	PAx = mirvar(0);
	PAy = mirvar(0);
	bytes_to_big(SM2_n1UMWORD, PriKey, d);
	ecurve_mult(d, G, PA);
	epoint_get(PA, PAx, PAy);
	big_to_bytes(SM2_n1UMWORD, PAx, Px, TRUE);
	big_to_bytes(SM2_n1UMWORD, PAy, Py, TRUE);
	i = Test_PubKey(PA);
	if (i)
		return i;
	else
	...
	char d[], unsigned char R[], unsigned char S[])
{
	...
	z1 = mirvar(0);
	z2 = mirvar(0);
	bytes_to_big(SM2_n1UMWORD, d, dA);//cinstr(dA,d);
	KG = epoint_init();
	//step1,set M=ZA||M
	...
	big_to_bytes(SM2_n1UMWORD, r, R, TRUE);
	big_to_bytes(SM2_n1UMWORD, s, S, TRUE);
	free(M);
	return 0;
}

/****************************************************************
Function: SM2_Verify///SM2验签
	...
	char Py[], unsigned char R[], unsigned char S[])
{
	...
	sG = epoint_init();
	tPA = epoint_init();
	bytes_to_big(SM2_n1UMWORD, Px, PAx);
	bytes_to_big(SM2_n1UMWORD, Py, PAy);
	bytes_to_big(SM2_n1UMWORD, R, r);
	bytes_to_big(SM2_n1UMWORD, S, s);
...
int SM2_SelfCheck()
{
	...
	unsigned char Msg[210]; //210=IDA_len+2+SM2_n1UMWORD*6
	int temp;
	...
	memcpy(Msg + 2 + IDA_len, SM2_a1, SM2_n1UMWORD);
	memcpy(Msg + 2 + IDA_len + SM2_n1UMWORD, SM2_b1, SM2_n1UMWORD);
	memcpy(Msg + 2 + IDA_len + SM2_n1UMWORD * 2, SM2_Gx1, SM2_n1UMWORD);
	memcpy(Msg + 2 + IDA_len + SM2_n1UMWORD * 3, SM2_Gy1, SM2_n1UMWORD);
	memcpy(Msg + 2 + IDA_len + SM2_n1UMWORD * 4, xA, SM2_n1UMWORD);
	memcpy(Msg + 2 + IDA_len + SM2_n1UMWORD * 5, yA, SM2_n1UMWORD);
	SM3_256(Msg, 210, ZA);
...
int SM2_testSign(char *message, unsigned char r[], unsigned char s[], unsigned char xA[], unsigned char yA[],unsigned char rand[],unsigned char dA[],unsigned char IDA[])
{
	...
	unsigned char ZA[SM3_len / 8];//ZA=Hash(ENTLA|| IDA|| a|| b|| Gx || Gy || xA|| yA)
	unsigned char Msg[226]; //210=IDA_len+2+SM2_n1UMWORD*6
	int temp;
	miracl *mip = mirsys(10000, 16);
	mip->IOBASE = 16;
	temp = SM2_KeyGeneration(dA, xA, yA);
	...
	printf("签名完毕!\n");
	printf("r is \n");
	for (int i = 0; i < 32; i++)//尝试直接传输r,s数组
	{
		printf("%02x", r[i]);
	}
	printf("\n***********************************\n");
